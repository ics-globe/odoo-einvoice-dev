...
def url_lang(path_or_uri, lang_code=None):
    ...
    if not url.netloc and not url.scheme and (url.path or force_lang):
        ...
        lang_code = pycompat.to_text(lang_code or request.env.context['lang'])
...
class ModelConverter(ir_http.ModelConverter):
    ...
    def to_python(self, value):
        ...
        env = request.env(user=RequestUID(value=value, match=matching, converter=self))
        if record_id < 0:
...
class IrHttp(models.AbstractModel):
    ...
    def _match(cls, path):
        ...
        7/ Rewrite the URL when the lang is present in the URL, that is
        ...
        elif not url_lang_str and cls.is_a_bot():
    ...
    def _pre_dispatch(cls, rule, args):
        ...
        if odoo._geoip_resolver is None:
        ...
        if request.is_frontend_multilang:
            # Inject the lang in the context of "<model(...):...>" args
            for key, val in list(args.items()):
                if isinstance(val, models.BaseModel):
                    args[key] = val.with_context(lang=request.lang.code)
    ...
    def url_rewrite(self, path, query_args=None):
        ...
        router = odoo.http.app.get_db_router(request.db).bind('')
        endpoint = False
        try:

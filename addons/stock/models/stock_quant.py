...
class StockQuant(models.Model):
    ...
    def _run_least_packages_removal_strategy_astar(self, domain, qty):
        ...
        query_str, params = query.select('package_id', 'SUM(quantity)')
        query_str += ' GROUP BY package_id ORDER BY SUM(quantity) DESC'
        self._cr.execute(query_str, params)
        qty_by_package = self._cr.fetchall()

        pkg_found = False
        # Items that do not belong to a package are added individually to the list
        ...
        def generate_domain(node):
            ...
            expr = [('package_id', 'in', [elem[0] for elem in node.taken_packages])]
        ...
        try:
            while not frontier.empty():
                ...
                while i < size:
                    ...
                    if count < 0:
                        # Overselect case
                        if best_leaf.count_remaining > 0 or len(node.taken_packages) < len(best_leaf.taken_packages):
                            best_leaf = node
                        continue

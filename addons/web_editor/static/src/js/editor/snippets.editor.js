...
var SnippetEditor = Widget.extend({
     ...
     * droppped in a sanitized HTML field.
     *
     * @param {Element} el
     * @return {boolean}
     */
...
var SnippetsMenu = Widget.extend({
    ...
    _computeSnippetTemplates: function (html) {
        ...
        this.$snippets = $scroll.find('.o_panel_body').children()
            ...
            .each(function () {
                ...
                const isSanitizeForbiddenClass = $snippet.data('oeForbidSanitize') ?
                    "oe_forbid_sanitize" : "";
                // Create the thumbnail
                const $existingThumbnail = $snippet.find('.oe_snippet_thumbnail');
                if ($existingThumbnail.length) {
                    if (isSanitizeForbiddenClass) {
                        $existingThumbnail[0].classList.add(isSanitizeForbiddenClass);
                    }
    ...
    _disableUndroppableSnippets: function () {
        ...
        this.$snippets.each(function () {
            ...
            function filterSanitize($els) {
                return isSanitizeForbidden ? $els.not('[data-oe-sanitize], [data-oe-sanitize] *') : $els;
            }
    ...
    _insertDropzone: function ($hook, vertical, style) {
        ...
        var $dropzone = $('<div/>', {
            'class': 'oe_drop_zone oe_insert' + (vertical ? ' oe_vertical' : '') +
                (forbidSanitize ? ' text-center bg-danger' : ''),
        });
        if (style) {
            $dropzone.css(style);
        }
        ...
        $snippets.draggable({
            ...
            start: function () {
                ...
                if ($snippet.data('oeForbidSanitize')) {
                    $dropZones = $dropZones.not('[data-oe-sanitize] .oe_drop_zone');
                }
                $dropZones.droppable({
                    over: function () {
                        if (dropped) {
    ...
    _onFindSnippetTemplate(ev) {
        this.$snippets.has('.oe_snippet_body').each(function () {
            ...
            let snippetClasses = snippetBody.className.match(/s_(?!parallax_)[^ ]+/g);
            if (snippetClasses) {

...
const Wysiwyg = Widget.extend({
    ...
    openMediaDialog(params = {}) {
        ...
        this.mediaDialogWrapper = new ComponentWrapper(this, MediaDialogWrapper, {
            ...
            save: (media) => {
                if (params.node) {
                    params.node.replaceWith(media);
                    this.odooEditor.unbreakableStepUnactive();
                    this.odooEditor.historyStep();
                } else {
                    this.odooEditor.execCommand('insertHTML', media.outerHTML);
                }
                restoreSelection();

                if (this.snippetsMenu) {
                    this.snippetsMenu.activateSnippet($(media)).then(() => {
                        if (media.tagName === 'IMG') {
                            $(media).trigger('image_changed');
                        }
                    });
                }
            },

...
class RTC {
    ...
    notifyClient(clientId, notificationName, notificationPayload, { transport = 'server' } = {}) {
        console.log(`notifyClient ${transport}:${notificationName}:${this.currentClientId}:${clientId}`);
        const transportPayload = {
            fromClientId: this._currentClientId,
            toClientId: clientId,
            notificationName,
    ...
    _channelNotify(clientId, transportPayload) {
        ...
        if (dataChannel.readyState !== 'open') {
            // throw new Error(`Client ${clientId} dataChannel.readyState is not open.`);
            console.warn(`Client ${clientId} dataChannel.readyState is not open.`);
        } else {

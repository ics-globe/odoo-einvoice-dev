    ...
    def _website_product_id_change(self, order_id, product_id, qty=0, **kwargs):
        ...
        if event_booth_pending_ids:
            ...
            if order.pricelist_id and order.partner_id:
                ...
                if order_line:
                    price_reduce = self.env['account.tax']._fix_tax_included_price_company(price_reduce, booths.product_id.taxes_id, order_line[0].tax_id, self.company_id)
            ...
            if order.pricelist_id.discount_policy == 'without_discount':
                ...
                if order.pricelist_id and order.partner_id:
                    order_line = order._cart_find_product_line(booths.product_id.id)
                    if order_line:
                        price = self.env['account.tax']._fix_tax_included_price_company(price, booths.product_id.taxes_id, order_line[0].tax_id, self.company_id)
                if price != 0:
                    if booth_currency != pricelist_currency:

...
class TestBlogPerformance(UtilPerf):
    ...
    def test_10_perf_sql_blog_standard_data(self):
        ...
        self.assertEqual(self._get_url_hot_query('/blog', cache=False), 12)
    ...
    def test_20_perf_sql_blog_bigger_data_scaling(self):
        ...
        self.assertEqual(self._get_url_hot_query('/blog'), 12)
        self.assertEqual(self._get_url_hot_query('/blog', cache=False), 12)
        self.assertEqual(self._get_url_hot_query(blog_post[0].website_url), 27)
        self.assertEqual(self._get_url_hot_query(blog_post[0].website_url, cache=False), 27)

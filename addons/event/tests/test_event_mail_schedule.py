...
class TestMailSchedule(EventCase, MockEmail):
    ...
    def test_unique_event_mail_ids(self):
        ...
        event_mail_ids = test_event.event_mail_ids
        ...
        self.assertTrue(mail_done in test_event.event_mail_ids, "Sent communication should not be removed")
        for mail in event_mail_ids - mail_done:
            self.assertTrue(mail not in test_event.event_mail_ids, "Other default communication lines should be removed")
        ...
        self.assertEqual(len(duplicate_mails), 0,
            "The duplicate configuration (first one from event_type.event_type_mail_ids which has same configuration as the sent one) should not be added")

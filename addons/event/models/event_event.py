...
class EventEvent(models.Model):
    ...
    def _compute_event_registrations_sold_out(self):
        ...
        for event in self:
            event.event_registrations_sold_out = \
                event.seats_limited and not event.seats_available \
    ...
    def _check_seats_availability(self, at_least=0):
        sold_out_events = []
        for event in self:
            if event.seats_limited and event.seats_max and event.seats_available < at_least:
                sold_out_events.append(
                    (_('- the event "%(event_name)s" (%(nb_too_many)i too many seats requested).',
                        event_name=event.name, nb_too_many=-event.seats_available)))
        if sold_out_events:
            raise ValidationError(_('There are not enough seats available with the current limits for:')

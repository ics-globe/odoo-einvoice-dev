...
from odoo.tools import get_html_content
...
class Message(models.Model):
    ...
    * snailmail.letter (snailmail addon)
    ...
    hidden_preview = fields.Char(
        'Hidden preview', compute='_compute_hidden_preview',
    ...
    @api.depends('body')
    def _compute_hidden_preview(self):
        for message in self:
            message.hidden_preview = message._get_message_preview()
    ...
    def _get_message_preview(self, max_char=190):
        ...
        plaintext_ct = get_html_content(self.body)
        if not max_char or len(plaintext_ct) <= max_char:
            return plaintext_ct
        return plaintext_ct[:max_char] + ' [...]'

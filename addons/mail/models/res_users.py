...
class Users(models.Model):
    ...
    def _deactivate_portal_user(self, **post):
        ...
        """
        for user in self:
            user.partner_id._message_log(body=_(
                'This partner was archived because the related user "%s" has requested to deactivate his account.',
                user.name))

        if post.get('blacklist'):
            emails_to_blacklist = filter(bool, [tools.email_normalize(user.email) for user in self])
        else:
            emails_to_blacklist = []

        super(Users, self)._deactivate_portal_user(**post)

        for email in emails_to_blacklist:
            blacklist = self.env['mail.blacklist']._add(email)
            blacklist._message_log(body=_('The email has been blacklisted because its related portal user has deactivated his account.'))

...
class DiscussController(http.Controller):
    ...
    def mail_channel_partner_avatar_128(self, channel_id, partner_id, **kwargs):
        ...
        if channel_partner_sudo and channel_partner_sudo.env['mail.channel.partner'].search([('channel_id', '=', channel_id), ('partner_id', '=', partner_id)], limit=1):
            return request.env['ir.binary']._get_stream_from(partner_sudo, field='avatar_128', placeholder=placeholder).get_response()
        if request.env.user.share:
            return http.Stream.from_path(placeholder).get_response()
        return request.env['ir.binary']._get_stream_from(partner_sudo.sudo(False), field='avatar_128', placeholder=placeholder).get_response()
    ...
    def mail_channel_guest_avatar_128(self, channel_id, guest_id, **kwargs):
        ...
        if channel_partner_sudo and channel_partner_sudo.env['mail.channel.partner'].search([('channel_id', '=', channel_id), ('guest_id', '=', guest_id)], limit=1):
            return request.env['ir.binary']._get_stream_from(guest_sudo, field='avatar_128', placeholder=placeholder).get_response()
        if request.env.user.share:
            return http.Stream.from_path(placeholder).get_response()
        return request.env['ir.binary']._get_stream_from(guest_sudo.sudo(False), field='avatar_128', placeholder=placeholder).get_response()

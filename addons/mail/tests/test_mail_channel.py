...
class TestChannelAccessRights(MailCommon):
    ...
    def setUpClass(cls):
        ...
        # Pigs: channel with restrictions
        cls.group_public_restriction = Channel.create({
            'name': 'Pigs',
            'channel_type': 'channel',
            'group_public_id': cls.env.ref('base.group_user').id})

        # Chat: chat
        cls.group_chat = Channel.create({
            'name': 'Chat',
            'channel_type': 'chat'})

        # # Group: group chat
        # cls.group_group = Channel.create({
        #     'name': 'Group',
        #     'channel_type': 'group'})
    ...
    def test_access_public(self):
        # Read public group -> ko, restricted to employees
        with self.assertRaises(AccessError):
            self.env['mail.channel'].browse(self.group_public.id).read()
        # Read groups -> ko, restricted to employees
        with self.assertRaises(AccessError):
            self.env['mail.channel'].browse(self.group_public_restriction.id).read()
        # Read private -> ko, restricted to members
        with self.assertRaises(AccessError):
            self.env['mail.channel'].browse(self.group_chat.id).read()

        # Read a private group when being a member: ok
        self.group_chat.write({'channel_partner_ids': [(4, self.user_public.partner_id.id)]})
        self.env['mail.channel'].browse(self.group_chat.id).read()
    ...
    def test_access_employee(self):
        ...
        group_public_restriction = self.env['mail.channel'].browse(self.group_public_restriction.id)
        group_public_restriction.read()
        ...
        group_public_restriction.write({'name': 'modified'})

        # Employee unlink employee-based group: ok
        group_public_restriction.unlink()
        ...
        with self.assertRaises(AccessError):
            self.env['mail.channel'].browse(self.group_chat.id).write({'name': 're-modified'})

        # Employee cannot unlink private
        with self.assertRaises(AccessError):
            self.env['mail.channel'].browse(self.group_chat.id).unlink()

    @mute_logger('odoo.addons.base.models.ir_rule', 'odoo.models')
    @users('user_portal')
    def test_access_portal(self):
        with self.assertRaises(AccessError):
            self.env['mail.channel'].browse(self.group_chat.id).name

        self.group_chat.write({'channel_partner_ids': [(4, self.user_portal.partner_id.id)]})
        group_private_portal = self.env['mail.channel'].browse(self.group_chat.id)
        ...
        for partner in self.group_chat.message_partner_ids:
    ...
    def test_members(self):
        ...
        group_chat = self.env['mail.channel'].browse(self.group_chat.id)
        with self.assertRaises(AccessError):
            group_chat.read(['name'])

        with self.assertRaises(AccessError):
            self.env['mail.channel.partner'].create({
                'partner_id': self.env.user.partner_id.id,
                'channel_id': group_chat.id,
            })
...
class TestChannelInternals(MailCommon):
    ...
    def test_channel_members(self):
        ...
        channel.add_members(self.partner_employee.ids)
        self.assertEqual(channel.message_partner_ids, self.env['res.partner'])
        self.assertEqual(channel.channel_partner_ids, self.partner_employee)

        channel._action_remove_members(self.partner_employee)
        self.assertEqual(channel.message_partner_ids, self.env['res.partner'])
        self.assertEqual(channel.channel_partner_ids, self.env['res.partner'])
    ...
    def test_channel_recipients_channel(self):
        ...
        channel = self.env['mail.channel'].browse(self.test_channel.ids)
        channel.add_members((self.partner_employee | self.partner_admin).ids)
    ...
    def test_channel_user_synchronize(self):
        ...
        # Unsubscribe archived user not from public channels / group
        self.user_employee.active = False
        self.assertEqual(test_channel_group.channel_partner_ids, self.user_employee.partner_id | self.partner_employee_nomail)
        self.assertEqual(self.test_channel.channel_partner_ids, self.user_employee.partner_id | self.partner_employee_nomail)

        # Unsubscribe deleted user not from public channels  / group
    ...
    def test_channel_unsubscribe_auto(self):
        ...
        # Unsubscribe archived user not from channels and not from chat
        self.user_employee.active = False
        (test_chat | self.test_channel).invalidate_cache(fnames=['channel_partner_ids'])
        self.assertEqual(test_channel_group.channel_partner_ids, self.user_employee.partner_id | test_partner)
        ...
        # Unsubscribe deleted user not from public channels and not from chat
        test_user.unlink()
        self.assertEqual(test_channel_group.channel_partner_ids, self.user_employee.partner_id | test_partner)
        self.assertEqual(self.test_channel.channel_partner_ids, self.user_employee.partner_id | test_partner)
        self.assertEqual(test_chat.channel_partner_ids, self.user_employee.partner_id | test_partner)
    ...
    def test_channel_unfollow_should_not_post_message_if_the_partner_has_been_removed(self):
        ...
        # no message should be posted under partner_employee's name
        ...
        channel._action_unfollow(self.partner_employee)

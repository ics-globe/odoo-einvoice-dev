...
QUnit.module('mail', {}, function () {
    ...
    QUnit.test('many2one_avatar_user widget in list view', async function (assert) {
        ...
        const { openView, target: list } = await start({
            serverData: { views },
        });
        await openView({
        ...
        await dom.click(list.querySelector('.o_data_cell .o_m2o_avatar > img'));
    ...
    QUnit.test('many2many_avatar_user widget in form view', async function (assert) {
        ...
        const { openView, target: form } = await start({
            serverData: { views },
        });
        await openView({
            res_model: 'm2x.avatar.user',
    ...
    QUnit.test('many2many_avatar_user in kanban view', async function (assert) {
        ...
        const { openView, target: kanban } = await start({
            serverData: { views },
        });
        await openView({
            res_model: 'm2x.avatar.user',

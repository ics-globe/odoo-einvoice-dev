 ...
 */
 export async function startServer() {
    ...
    for (const [modelName, fields] of modelDefinitions) {
        ...
        for (const [viewType, viewArch] of Object.entries(VIEW_TYPES)) {
...
async function start(param0 = {}) {
    ...
    const openView = async (action, options) => {
        ...
        action['name'] = action['name'] || 'open_view';
    ...
    return {
        ...
        createFollowerListMenuComponent: getCreateFollowerListMenuComponent({ env: webClient.env, modelManager, target }),

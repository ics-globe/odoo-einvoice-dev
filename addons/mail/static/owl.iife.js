...
See https://github.com/odoo/owl/blob/master/doc/reference/config.md#mode for more information.`;
    ...
    const observerSourceAtom = new WeakMap();
    const SOURCE = Symbol("source");
    const OBSERVER = Symbol("observer");
    const KEYS = Symbol("keys");
    const ROOT = Symbol("root");
    const SEED = Symbol("Seed");
    function observe(source, observer) {
    ...
    function atom(source, observer) {
        return _atom(source, observer, true);
    }
    function _atom(source, observer, seed = false) {
    ...
    function unregisterObserverAtoms(observer, keepSeeds) {
    ...
    class ObserverSet {
        constructor() {
            this.nodeAntichain = new Antichain();
    ...
    function isLessOrEqual(node1, node2) {
        let current = node1;
    ...
    let toNotify = null;
    let toClean = new Set();
    async function notify(observers, clean = false) {
        if (clean) {
    ...
    __info__.date = '2021-11-17T16:21:35.296Z';
    __info__.hash = 'c2284bc';

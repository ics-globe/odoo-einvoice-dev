...
class TestEventPerformance(EventPerformanceCase):
    ...
    def test_event_create_form_type_website(self):
        ...
        # type and website
        with freeze_time(self.reference_now), self.assertQueryCount(event_user=724):  # tef only: 673 - com runbot: 624 - ent runbot: 724
            self.env.cr._now = self.reference_now  # force create_date to check schedulers
            with Form(self.env['event.event']) as event_form:
                event_form.name = 'Test Event'
                event_form.date_begin = self.reference_now + timedelta(days=1)
...
class TestRegistrationPerformance(EventPerformanceCase):
    ...
    def test_registration_create_batch(self):
        ...
        with freeze_time(self.reference_now), self.assertQueryCount(event_user=718):  # tef only: 672 - com runbot 715 - ent runbot 718
            self.env.cr._now = self.reference_now  # force create_date to check schedulers
            registration_values = [
                dict(reg_data,
                     event_id=event.id)
    ...
    def test_registration_create_batch_website(self):
        ...
        with freeze_time(self.reference_now), self.assertQueryCount(event_user=729):  # tef only: 683 - com runbot 726 - ent runbot: 729
            self.env.cr._now = self.reference_now  # force create_date to check schedulers
            registration_values = [
                dict(reg_data,
                     event_id=event.id)
    ...
    def test_registration_create_single_customer(self):
        ...
        with freeze_time(self.reference_now), self.assertQueryCount(event_user=141):  # tef only: 134 - com runbot: 139 - ent runbot: 141
            self.env.cr._now = self.reference_now  # force create_date to check schedulers
    ...
    def test_registration_create_single_partner(self):
        ...
        with freeze_time(self.reference_now), self.assertQueryCount(event_user=147):  # tef only: 141 - com runbot: 146 - ent runbot: 147
            self.env.cr._now = self.reference_now  # force create_date to check schedulers
            registration_values = {
                'event_id': event.id,
    ...
    def test_registration_create_single_website(self):
        ...
        with freeze_time(self.reference_now), self.assertQueryCount(event_user=149):  # tef only: 140 - com runbot: 145 - ent runbot: 149
            self.env.cr._now = self.reference_now  # force create_date to check schedulers
            registration_values = dict(

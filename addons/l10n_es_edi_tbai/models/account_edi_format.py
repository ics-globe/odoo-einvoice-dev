...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_es_tbai_get_invoice_values(self, invoice, cancel):
        ...
        if cancel:
            inv_details = inv_info['TipoDesglose']['DesgloseTipoOperacion']
        else:
            inv_details = inv_info['TipoDesglose']['DesgloseFactura']

        if invoice._is_l10n_es_tbai_simplified():
            values['regime_key'].append(52)
        elif any(filter(
                lambda l: any(filter(lambda d: d.get('TipoRecargoEquivalencia'), l.get('Sujeta', {}).get('NoExenta', {}).get('DesgloseIVA', {}).get('DetalleIVA', {}))),
                (inv_details, inv_details.get("PrestacionServicios", {}), inv_details.get("Entrega", {}))
        )):
            values['regime_key'].append(51)

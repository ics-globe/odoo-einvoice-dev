...
class TestUBLDE(TestUBLCommon):
    ...
    def test_export_pdf(self):
        ...
        self.assertEqual(self.invoice.attachment_ids.mapped("name"), ['INV_2017_01_0001_ubl_de.xml'])
    ...
    def test_import_invoice_xml(self):
        ...
        self.update_invoice_from_file('account_edi_ubl_cii_tests', 'test_files', 'test_de_out_invoice.xml', invoice)

        self.assertEqual(len(self.env['account.move'].search([])), invoice_count)
        self.assertEqual(invoice.amount_total, 3083.58)  # see cbc:PayableAmount

        self.create_invoice_from_file('account_edi_ubl_cii_tests', 'test_files', 'test_de_out_invoice.xml')

        self.assertEqual(invoice.amount_total, 3083.58)

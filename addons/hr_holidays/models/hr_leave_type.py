...
class HolidaysType(models.Model):
    ...
    def _is_valid(self, date_from, date_to, employee_id):
        ...
        for holiday_type in self:
            if employee_id and holiday_type.requires_allocation and date_from and date_to:
                allocation = allocations.filtered_domain([
                    ('holiday_status_id', '=', holiday_type.id)
                ])
                holiday_type.sudo().has_valid_allocation = bool(allocation)
            else:
                holiday_type.sudo().has_valid_allocation = True
    ...
    def _compute_valid(self):
        ...
        self._is_valid(date_from, date_to, employee_id)

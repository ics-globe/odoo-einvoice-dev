...
class DigestController(Controller):
    ...
    def digest_unsubscribe(self, digest_id, token=None, user_id=None, one_click=None):
        ...
        :param one_click: set it to 1 when using the URL in the header of the email to allow mail user agent to
        propose a one click button to the user to unsubscribe as defined in rfc8058.
        When set to True, only POST method will be allowed preventing the risk that anti-spam trigger unwanted
        unsubscribe (scenario explained in the same rfc).
        Note: this method must support encoding method 'multipart/form-data' and 'application/x-www-form-urlencoded'."""

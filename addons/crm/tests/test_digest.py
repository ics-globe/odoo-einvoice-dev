...
class TestCrmDigest(TestDigestCommon):
    ...
    def setUpClass(cls):
        super(TestCrmDigest, cls).setUpClass()
    ...
    def test_kpi_crm_lead_created_value(self):
        ...
        self.digest_3.invalidate_cache(ids=self.digest_3.ids)
        self.assertEqual(
            self.digest_3.with_company(self.company_2).kpi_crm_lead_created_value,
            0,
        )

    def test_kpi_crm_opportunities_won_value(self):
        self.assertEqual(self.digest_1.kpi_crm_opportunities_won_value, 1)
        self.assertEqual(self.digest_2.kpi_crm_opportunities_won_value, 0,
            msg='This digest is in a different company')
        self.assertEqual(self.digest_3.kpi_crm_opportunities_won_value, 1,
            msg='This digest has no company, should take the current one')

        self.digest_3.invalidate_cache(ids=self.digest_3.ids)
        self.assertEqual(
            self.digest_3.with_company(self.company_2).kpi_crm_opportunities_won_value,
            0,
        )

...
class Team(models.Model):
    ...
    def _action_update_to_pipeline(self, action):
        ...
        if user_team_id:
            ...
            action['help'] = _("<p class='o_view_nocontent_smiling_face'>Create an Opportunity</p>")
            if user_team_id:
                if self.user_has_groups('sales_team.group_sale_manager'):
                    action['help'] += _("""<p>As you are a member of no Sales Team, you are showed the Pipeline of the <b>first team by default</b>.
                                        To work with the CRM, you should <a name="%d" type="action" tabindex="-1">join a team.</a></p>""",
                                        self.env.ref('sales_team.crm_team_action_config').id)
                else:
                    action['help'] += _("""<p>As you are a member of no Sales Team, you are showed the Pipeline of the <b>first team by default</b>.
                                        To work with the CRM, you should join a team.</p>""")

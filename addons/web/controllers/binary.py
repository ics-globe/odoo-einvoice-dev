...
from odoo.tools import file_open, file_path, reraise_x_as
...
Odoo is running with --x-sendfile but is receiving /web/filesystem requests.

With --x-sendfile enabled, a reverse proxy (nginx, apache)
should be serving the /web/filesystem route, however Odoo
is receiving the request.
This usually indicates that the reverse proxy is badly
configured and that binary files are not being served
properly.

Make sure the /web/filesystem location block exists in
your reverse proxy and contains a configuration similar
to (in the case of nginx):
...
class Binary(http.Controller):
    ...
    def content_filesystem(self, _path):
        if odoo.tools.config['x_sendfile']:
            _logger.error(BAD_X_SENDFILE_ERROR)
    ...
    def content_common(self, xmlid=None, model='ir.attachment', id=None, field='raw',
        ...
        with reraise_x_as(UserError, as_=request.not_found()):
            record = request.env['ir.binary']._find_record(xmlid, model, id, access_token)
    ...
    def content_assets(self, id=None, filename=None, unique=False, extra=None, nocache=False):
        if not id:
            ...
            id = attachments[0]['id']
        with reraise_x_as(UserError, as_=request.not_found()):
            record = request.env['ir.binary']._find_record(res_id=id)

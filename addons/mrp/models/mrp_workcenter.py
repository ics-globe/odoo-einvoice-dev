...
class MrpWorkCenterCapacity(models.Model):
    ...
    capacity = fields.Float('Capacity', default=1.0, help="Number of pieces of this product that can be produced in parallel.")

    _sql_constraints = [
        ('positive_capacity', 'CHECK(capacity > 0)', 'Capacity should be a positive number.'),
        ('unique_product', 'UNIQUE(workcenter_id, product_id)', 'Multiple capacities.'),
    ]

...
class MrpProduction(models.Model):
    ...
    def copy(self, default=None):
        ...
        for workorder in self.workorder_ids:
            if workorder.blocked_by_workorder_ids:
                copied_workorder = res.workorder_ids.filtered(lambda wo: wo.operation_id == workorder.operation_id)
                dependencies = []
                for dependency in workorder.blocked_by_workorder_ids:
                    dependencies.append(Command.link(res.workorder_ids.filtered(lambda wo: wo.operation_id == dependency.operation_id).id))
    ...
    def _link_workorders_and_moves(self):
        ...
        if self.allow_workorder_dependencies:
            ...
            previous_workorder = self.env['mrp.production']
            for workorder in self.workorder_ids:
        ...
        boms = set(workorder.operation_id.bom_id for workorder in self.workorder_ids)
        last_workorder_per_bom = {bom: self.workorder_ids.filtered(lambda wo: wo.operation_id.bom_id == bom and not wo.needed_by_workorder_ids)[-1] for bom in boms}
        for move in (self.move_raw_ids | self.move_finished_ids):
            if move.operation_id:
                ...
                bom = move.bom_line_id.bom_id if move.bom_line_id else self.bom_id
                if bom not in boms:
                    bom = self.bom_id
                if bom in boms:
                    move.write({

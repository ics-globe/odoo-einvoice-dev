...
class MassMailing(models.Model):
        ...
        url = werkzeug.urls.url_join(
            self.get_base_url(), 'mail/mailing/%(mailing_id)s/unsubscribe?%(params)s' % {
                ...
                'params': werkzeug.urls.url_encode({
                    'res_id': res_id,
                    'email': email_to,
                    'token': self._generate_mailing_recipient_hash(res_id, email_to),
                }),
            }
        )
        return url
    ...
    def _action_send_statistics(self):
        ...
        for mailing in self:
            ...
            if mail_user.has_group('mass_mailing.group_mass_mailing_user'):
                rendering_data['mailing_report_token'] = self._generate_mailing_report_hash(mail_user.id)
    ...
    def _generate_mailing_report_hash(self, user_id):

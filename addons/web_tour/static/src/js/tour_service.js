...
return session.is_bound.then(function () {
    ...
    return Promise.all(defs).then(function (results) {
        ...
        // window.
        const isInIframe = window.frameElement && window.frameElement.matches(`o_iframe`);
        if (isInIframe) {
            return tour_manager;
        }
                ...
                return new Promise(function (resolve, reject) {
                    tour_manager._register_all(observe).then(function () {
                        if (observe) {
                            ...
                            // tour manager.
                            const observableIframeSelector = '.o_iframe:not(.o_technical_iframe)';
                            const containsOrIsIframe = node => {
                                const isElement = node.nodeType === 1;
                                return isElement && (node.querySelector(observableIframeSelector) || node.matches(observableIframeSelector));
                            };
                            const iframeObserver = new MutationObserver(mutations => {
                                const iframeMutation = mutations.filter(mutation => Array.from(mutation.addedNodes).some(containsOrIsIframe))[0];
                                if (iframeMutation) {
                                    let iframeEl = iframeMutation.target.querySelector(observableIframeSelector);
                                    if (iframeMutation.target.matches(observableIframeSelector)) {
                                        iframeEl = iframeMutation.target;
                                    }

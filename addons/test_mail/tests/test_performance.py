...
class BaseMailPerformance(TransactionCaseWithUserDemo):
    ...
    def _init_mail_gateway(self):
        ...
        self.env['mail.alias.domain'].search([('company_id', '=', False)]).unlink()
        self.alias_domain_global = self.env['mail.alias.domain'].create({
            'bounce': self.alias_bounce,
            'catchall': self.alias_catchall,
            'name': self.alias_domain,
        })
...
class TestBaseMailPerformance(BaseMailPerformance):
    ...
    def test_create_mail_with_tracking(self):
        ...
        with self.assertQueryCount(__system__=9, demo=9):
...
class TestMailAPIPerformance(BaseMailPerformance):
    ...
    def test_message_assignation_email(self):
        ...
        with self.assertQueryCount(__system__=39, employee=40):
            record.write({
                'user_id': self.user_test.id,
            })
...
class TestMailComplexPerformance(BaseMailPerformance):
    ...
    def test_complex_message_post(self):
        ...
        with self.assertQueryCount(__system__=64, employee=65):
            record.message_post(
    ...
    def test_complex_message_post_template(self):
        ...
        with self.assertQueryCount(__system__=73, employee=74):
    ...
    def test_complex_tracking_assignation(self):
        ...
        with self.assertQueryCount(__system__=40, employee=41):
    ...
    def test_complex_tracking_subscription_create(self):
        ...
        with self.assertQueryCount(__system__=111, employee=112):
            rec = self.env['mail.test.ticket'].create({
                'name': 'Test',
    ...
    def test_complex_tracking_subscription_write(self):
        ...
        with self.assertQueryCount(__system__=86, employee=86):
            rec.write({
                'name': 'Test2',
                'container_id': container_id,
                'customer_id': customer_id,
    ...
    def test_complex_tracking_template(self):
        ...
        with self.assertQueryCount(__system__=30, employee=31):
            rec.write({
                'name': 'Test2',
                'customer_id': customer_id,
                'user_id': self.env.uid,
...
class TestMailHeavyPerformancePost(BaseMailPerformance):
    ...
    def test_complete_message_post(self):
        ...
        with self.assertQueryCount(employee=71):

...
QUnit.test('basic rendering of tracking value (float type)', async function (assert) {
    ...
    assert.containsOnce(
        document.body,
        '.o_MessageTrackingValue',
        "should display a tracking value"
    );
    assert.containsOnce(
        document.body,
        '.o_MessageTrackingValue_changedField',
        "should display the name of the tracked field"
    );
    assert.strictEqual(
        document.querySelector('.o_MessageTrackingValue_changedField').textContent,
        "Float:",
        "should display the correct tracked field name (Float)",
    );
    assert.containsOnce(
        document.body,
        '.o_MessageTrackingValue_oldValue',
        "should display the old value"
    );
    assert.strictEqual(
        document.querySelector('.o_MessageTrackingValue_oldValue').textContent,
        "12.30",
        "should display the correct old value (12.30)",
    );
    assert.containsOnce(
        document.body,
        '.o_MessageTrackingValue_arrowIcon',
        "should display the separator"
    );
    assert.containsOnce(
        document.body,
        '.o_MessageTrackingValue_newValue',
        "should display the new value"
    );
    assert.strictEqual(
        document.querySelector('.o_MessageTrackingValue_newValue').textContent,
        "45.67",
...
QUnit.test('rendering of tracked field of type float: from non-0 to 0', async function (assert) {
    ...
    await afterNextRender(() => testUtils.form.clickSave(form));
    assert.strictEqual(
        document.querySelector('.o_MessageTrackingValue').textContent,
        "Float:1.000.00",

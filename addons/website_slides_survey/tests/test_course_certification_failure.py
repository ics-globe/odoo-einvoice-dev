...
class TestCourseCertificationFailureFlow(TestSurveyCommon):
    def test_course_certification_failure_flow(self):
        ...
        self.assertFalse(channel_partner.active, 'Public user should have been archived from the course attendee because he failed his last attempt')
        # Step 7: add public user as member of the channel once again
        self.channel._action_add_members(self.user_public.partner_id)
        # forces recompute of partner_ids as we create directly in relation
        self.channel.invalidate_cache()
        self.slide_certification.with_user(self.user_public)._generate_certification_url()
        self.assertTrue(channel_partner.active, 'Public user should be a unarchived upon joining the course once again')
        self.assertEqual(1, len(slide_partner.user_input_ids.filtered(lambda user_input: user_input.state != 'done')), 'A new user input should have been automatically unarchived upon joining course again')

        # Step 8: fill in the created user_input with correct answers this time
        self.fill_in_answer(slide_partner.user_input_ids.filtered(lambda user_input: user_input.state != 'done')[0], certification.question_ids, good_answers=True)
        self.assertTrue(slide_partner.survey_scoring_success, 'Quizz should be marked as passed with correct answers')
        # forces recompute of partner_ids as we delete directly in relation
        self.channel.invalidate_cache()
        self.assertTrue(channel_partner.active, 'Public user should be active')

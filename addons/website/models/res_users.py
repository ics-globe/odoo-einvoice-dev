...
class ResUsers(models.Model):
    ...
    def authenticate(cls, db, login, password, user_agent_env):
        ...
        if uid and visitor_sudo:
            with cls.pool.cursor() as cr:
                env = api.Environment(cr, uid, {})
                user_partner = env.user.partner_id
                visitor_user_sudo = env['website.visitor'].sudo().search([
                    ('partner_id', '=', user_partner.id)
                ], limit=1)
                if visitor_user_sudo:
                    # A visitor exists for the logged in user, link public
                    # visitor records to it.
                    visitor_sudo._merge_visitor(visitor_user_sudo)
                    visitor_user_sudo._update_visitor_last_visit()
                else:
                    visitor_sudo.access_token = user_partner.id

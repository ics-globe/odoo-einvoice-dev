...
from odoo.websocket import Websocket, ws_request
...
class ImDispatch(threading.Thread):
    ...
    def subscribe(self, channels, last):
        channels = [channel_with_db(ws_request.db, c) for c in channels]
        subscription = self._ws_to_subscription.get(ws_request.ws)
        if subscription:
            last = subscription.last_notification_id
        subscription = BusSubscription(ws_request.db, channels, last)
        self._ws_to_subscription[ws_request.ws] = subscription
        if not self.is_alive():
            self.start()
        # Dispatch past notifications if there are any
        self._dispatch_notifications(ws_request.ws, subscription)

    def unsubscribe(self):
        self._ws_to_subscription.pop(ws_request.ws, None)
            ...
            while True:
                ...
                while conn.notifies:
                    notified_channels.update(
                        hashable(channel) for channel in json.loads(conn.notifies.pop().payload))

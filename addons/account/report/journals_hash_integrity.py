...
class ReportAccountHashIntegrity(models.AbstractModel):
    _name = 'report.account.report_journals_hash_integrity'
    _description = 'Get hash integrity result as PDF.'

    @api.model
    def _get_report_values(self, docids, data=None):
        if data:
            data.update(self.env.company._check_journals_hash_integrity())
        else:
            data = self.env.company._check_journals_hash_integrity()
        return {
            'doc_ids': docids,

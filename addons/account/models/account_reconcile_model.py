...
class AccountReconcileModelLine(models.Model):
    ...
    * From Label: There is no need for regex delimiter, only the regex is needed. For instance if you want to extract the amount from\nR:9672938 10/07 AX 9415126318 T:5L:NA BRT: 3358,07 C:\nYou could enter\nBRT: ([\d,]+)""")
    ...
    def _apply_line_for_manual_widget(self, residual_amount_currency, partner, currency):
        self.ensure_one()
    ...
    def _apply_line_for_bank_widget(self, residual_amount_currency, partner, st_line):
        self.ensure_one()
        ...
        if self.amount_type == 'percentage_st_line':
            ...
            if match:
                ...
                try:
                    extracted_balance = float(re.sub(r'\D' + decimal_separator, '', match.group(1)).replace(decimal_separator, '.'))
        ...
        if amount_currency is None:
            aml_vals = self._apply_line_for_manual_widget(residual_amount_currency, partner, currency)
        else:
            aml_vals = {
                **self._prepare_aml_vals(partner),
                'currency_id': currency.id,
...
class AccountReconcileModel(models.Model):
    ...
    def _apply_lines_for_bank_widget(self, residual_amount_currency, partner, st_line):
        ...
        for line in self.line_ids:
            vals = line._apply_line_for_bank_widget(residual_amount_currency, partner, st_line)
                ...
                for rule_index in sorted(rules_map.keys()):
                    for rule_method in rules_map[rule_index]:
                        ...
                        if candidate_vals.get('amls'):
                            ...
                            result['model'] = rec_model
                            result.update(candidate_vals)
                            return result
    ...
    def _get_invoice_matching_st_line_strings(self, st_line):

        def _get_text_value(field_name):
            if st_line._fields[field_name].type == 'html':
                return tools.html2plaintext(st_line[field_name])
            else:
                return st_line[field_name]
    ...
    def _get_invoice_matching_st_line_tokens(self, st_line):
        ...
        """
        st_line_text_values = self._get_invoice_matching_st_line_strings(st_line)
        ...
        for text_value in st_line_text_values:
            for token in (text_value or '').split():
                ...
                formatted_token = ''.join(x for x in token if x.isdigit())
    ...
    def _get_invoice_matching_amls_result(self, st_line, partner, candidate_vals):
        ...
        if set(amls.currency_id.ids) == {st_line_currency.id}:
    ...
    def _check_rule_propositions(self, st_line, amls):
        ...
        amls_amount_curr = sum(
            st_line._convert_to_st_line_rate(aml.currency_id, aml.amount_residual, aml.amount_residual_currency)['amount_currency']
            for aml in amls
        )
        ...
        reconciled_percentage_left = (abs(amount_curr_after_rec) / abs(amls_amount_curr)) * 100.0

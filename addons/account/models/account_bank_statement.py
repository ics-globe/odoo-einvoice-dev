...
class AccountBankStatementLine(models.Model):
    ...
    def _convert_to_st_line_rate(self, currency, balance, amount_currency):
        self.ensure_one()

        journal = self.journal_id
        company_currency = journal.company_id.currency_id
        journal_currency = journal.currency_id or company_currency
        trans_currency = self.foreign_currency_id or journal_currency or company_currency

        journal_amount = self.amount
        if trans_currency == journal_currency:
        ...
        elif trans_currency == company_currency:
        ...
        rate_journal_trans_curr = journal_amount and abs(transaction_amount) / abs(journal_amount)
        rate_comp_journal_curr = company_amount and abs(journal_amount) / abs(company_amount)

        if currency == trans_currency:
            trans_amount_currency = amount_currency
            if rate_journal_trans_curr:
                journ_amount_currency = journal_currency.round(trans_amount_currency / rate_journal_trans_curr)
            else:
                journ_amount_currency = 0.0
            if rate_comp_journal_curr:
                new_balance = company_currency.round(journ_amount_currency / rate_comp_journal_curr)
            else:
                new_balance = 0.0
        elif currency == journal_currency:
            trans_amount_currency = trans_currency.round(amount_currency * rate_journal_trans_curr)
            if rate_comp_journal_curr:
                new_balance = company_currency.round(amount_currency / rate_comp_journal_curr)
            else:
                new_balance = 0.0
        else:
            journ_amount_currency = journal_currency.round(balance * rate_comp_journal_curr)
            trans_amount_currency = trans_currency.round(journ_amount_currency * rate_journal_trans_curr)
            new_balance = balance
        ...
        if self.payment_ref:
            ...
            def _get_text_value(field_name):
                if not self[field_name]:
                    return False
                elif self._fields[field_name].type == 'html':
                    return html2plaintext(self[field_name])
                else:
                    return self[field_name]

            st_line_text_values = [
                _get_text_value('payment_ref'),
                _get_text_value('narration'),
                _get_text_value('ref'),
            ]
            ...
            if sub_queries:
                self.env['res.partner'].flush(['company_id', 'name'])
                self._cr.execute(
                    '''
                        SELECT id
                        FROM res_partner

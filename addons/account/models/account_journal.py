...
class AccountJournal(models.Model):
    ...
    def _get_last_bank_statement(self, domain=None):
        ...
        last_st_lines = self.env['account.bank.statement.line'].search(last_statement_domain)
        last_statement = self.env['account.bank.statement'].search([('id', 'in', last_st_lines.mapped('statement_id.id'))], order='date desc, id desc', limit=1)

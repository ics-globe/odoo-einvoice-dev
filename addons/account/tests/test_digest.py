...
class TestAccountDigest(TestDigestCommon):
    ...
    def setUpClass(cls):
        super(TestAccountDigest, cls).setUpClass()
        account = cls.env['account.account'].search([('internal_group', '=', 'income')], limit=1)
        ...
        moves = cls.env['account.move'].create({
            'line_ids': [
                (0, 0, {'debit': 5, 'credit': 0, 'account_id': account.id}),
                (0, 0, {'debit': 0, 'credit': 5, 'account_id': account2.id}),
                (0, 0, {'debit': 8, 'credit': 0, 'account_id': account.id}),
    ...
    def test_kpi_account_total_revenue_value(self):
        self.assertEqual(
            self.all_digests.mapped('kpi_account_total_revenue_value'),
            [-13.0, 2.0, -13.0],
        )

...
class TestAccountBankStatementLine(TestAccountBankStatementCommon):
    ...
    def test_convert_to_st_line_rate(self):

        def assertAppliedRate(
                journal_currency, trans_currency, aml_currency,
            ...
            statement = self.env['account.bank.statement'].create({
                ...
                'line_ids': [
                    Command.create({
                        'date': '2019-01-01',
                        'payment_ref': 'test_convert_to_st_line_rate',
                        'foreign_currency_id': trans_currency.id if trans_currency != journal_currency else None,
                        'amount': amount,
                        'amount_currency': amount_currency if trans_currency != journal_currency else 0.0,
                    }),
                ],
            })
            statement_line = statement.line_ids

            res = statement_line._convert_to_st_line_rate(aml_currency, -aml_balance, -aml_amount_currency)
        ...
        assertAppliedRate(
            ...
            80.0, 120.0, 120.0, 20.0,
            -120.0, -40.0,
        )
        assertAppliedRate(
            self.currency_2, self.currency_1, self.currency_2,
            120.0, 80.0, 120.0, 40.0,
            -80.0, -80.0,
        )
        assertAppliedRate(
            self.currency_2, self.currency_3, self.currency_2,
            80.0, 120.0, 80.0, 26.67,
            -120.0, -40.0,
        )
        assertAppliedRate(
            self.currency_2, self.currency_3, self.currency_4,
            80.0, 120.0, 480.0, 40.0,
            -120.0, -40.0,
        )
        assertAppliedRate(
            self.currency_1, self.currency_2, self.currency_2,
            80.0, 120.0, 120.0, 40.0,
            -120.0, -80.0,
        )
        assertAppliedRate(
            self.currency_1, self.currency_2, self.currency_3,
            80.0, 120.0, 480.0, 80.0,
            -120.0, -80.0,
        )
        assertAppliedRate(
            self.currency_2, self.currency_2, self.currency_2,
            80.0, 80.0, 80.0, 26.67,
            -80.0, -40.0,
        )
        assertAppliedRate(
            self.currency_2, self.currency_2, self.currency_3,
            80.0, 80.0, 240.0, 40.0,
            -80.0, -40.0,
        )
        assertAppliedRate(
            self.currency_1, self.currency_1, self.currency_3,
            80.0, 80.0, 480.0, 80.0,
            -80.0, -80.0,
        )
        assertAppliedRate(
            self.currency_2, self.currency_1, self.currency_1,
            120.0, 80.0, 80.0, 80.0,
            -80.0, -80.0,
        )
        assertAppliedRate(
            self.currency_2, self.currency_3, self.currency_1,
            80.0, 120.0, 40.0, 40.0,
            -120.0, -40.0,
        )
        assertAppliedRate(
        ...
        assertAppliedRate(
            self.currency_2, self.currency_2, self.currency_1,
            80.0, 80.0, 40.0, 40.0,
            -80.0, -40.0,
        )
        assertAppliedRate(
            self.currency_1, self.currency_1, self.currency_1,
            80.0, 80.0, 80.0, 80.0,
            -80.0, -80.0,
        )

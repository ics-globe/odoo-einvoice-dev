...
class PurchaseOrder(models.Model):
    ...
    def create(self, vals_list):
        ...
        if self.env.context.get('origin_po_id', False):
            ...
            if origin_po_id.purchase_group_id:
                origin_po_id.alternative_po_ids |= orders
            else:
    ...
    def write(self, vals):
        ...
        if vals.get('alternative_po_ids', False):
            if not self.purchase_group_id and len(self.alternative_po_ids + self) > 1:
        ...
        if vals.get('purchase_group_id', False):
            # the write is for multiple POs => don't double count the the POs of the final group
    ...
    def action_compare_alternative_lines(self):
        best_price_ids, best_date_ids = self.get_tender_best_date_and_price_lines()
        ...
        return {
            ...
            'views': [(view_id, "list")],
            'domain': [('order_id', 'in', self.ids + self.alternative_po_ids.ids)],
            'context': ctx,
        }

    def get_tender_best_date_and_price_lines(self):
        ...
        return best_price_ids, best_date_ids

...
import { clear, create, link, unlink, update } from '@discuss/model/model_field_command';

function factory(dependencies) {

    class ChatWindow extends dependencies['discuss.model'] {
        ...
        close({ notifyServer } = {}) {
            ...
            if (channel && notifyServer) {
                channel.notifyFoldStateToServer('closed');
            }
        ...
        expand() {
            if (this.channel) {
                this.channel.open({ expanded: true });
            }
        }
        ...
        fold({ notifyServer } = {}) {
            ...
            if (this.channel && notifyServer) {
                this.channel.notifyFoldStateToServer('folded');
            }
        }
        ...
        unfold({ notifyServer } = {}) {
            ...
            // new state on the server.
            if (this.channel && notifyServer) {
                this.channel.notifyFoldStateToServer('open');
            }
        }
        ...
        _computeHasNewMessageForm() {
            return this.isVisible && !this.isFolded && !this.channel;
        }
        ...
        _computeIsFolded() {
            const channel = this.channel;
            if (channel) {
                return channel.foldState === 'folded';
            }
        ...
        async _onHideHomeMenu() {
            ...
            this.channelView.addComponentHint('home-menu-hidden');
        }
        ...
        async _onShowHomeMenu() {
            ...
            this.channelView.addComponentHint('home-menu-shown');
        }
    ...
    ChatWindow.fields = {
        ...
        manager: many2one('discuss.chat_window_manager', {
            inverse: 'chatWindows',
        }),
        managerAllOrderedVisible: one2many('discuss.chat_window', {
         ...
         * If no `discuss.channel` is linked, `this` is considered "new message".
         */
        channel: one2one('discuss.channel', {
            inverse: 'chatWindow',
        }),
        channelDisplayName: attr({
    ...
    ChatWindow.modelName = 'discuss.chat_window';

    return ChatWindow;
}

registerNewModel('discuss.chat_window', factory);

...
QUnit.test('create (txt)', async function (assert) {
    ...
    const attachment = this.env.models['ir.attachment'].create({
        filename: "test.txt",
        id: 750,
        mimetype: 'text/plain',
        name: "test.txt",
    ...
    assert.strictEqual(this.env.models['ir.attachment'].findFromIdentifyingData({ id: 750 }), attachment);
...
QUnit.test('displayName', async function (assert) {
    ...
    assert.ok(attachment);
    assert.ok(this.env.models['ir.attachment'].findFromIdentifyingData({ id: 750 }));
    assert.strictEqual(attachment, this.env.models['ir.attachment'].findFromIdentifyingData({ id: 750 }));
...
QUnit.test('fileType', async function (assert) {
    ...
    assert.ok(attachment);
    assert.ok(this.env.models['ir.attachment'].findFromIdentifyingData({ id: 750 }));
    assert.strictEqual(attachment, this.env.models['ir.attachment'].findFromIdentifyingData({
        id: 750,
    }));

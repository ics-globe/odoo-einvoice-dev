...
import { link, replace, unlink } from '@discuss/model/model_field_command';
...
function factory(dependencies) {
    ...
    class ChatWindowManager extends dependencies['discuss.model'] {
        ...
        /**
         * Closes all chat windows related to the given channel.
         *
         * @param {discuss.channel} channel
         * @param {Object} [options]
         */
        closeChannel(channel, options) {
            for (const chatWindow of this.chatWindows) {
                if (chatWindow.channel === channel) {
        ...
        openNewMessage() {
            ...
            if (!newMessageChatWindow) {
                newMessageChatWindow = this.env.models['discuss.chat_window'].create({
                    manager: link(this),
                });
            }
        ...
        openChannel(channel, {
            ...
            if (!chatWindow) {
                chatWindow = this.env.models['discuss.chat_window'].create({
                    isFolded,
                    manager: link(this),
            ...
            if (notifyServer) {
                ...
                channel.notifyFoldStateToServer(foldState);
            }
        }
         ...
         *
         * @param {discuss.chat_window} chatWindow
         */
        ...
        /**
         * @param {discuss.chat_window} chatWindow1
         * @param {discuss.chat_window} chatWindow2
         */
        ...
        _compute_ordered() {
            ...
            const _ordered = this._ordered.filter(chatWindowLocalId =>
                this.chatWindows.includes(this.env.models['discuss.chat_window'].get(chatWindowLocalId))
            );
        ...
        _computeAllOrdered() {
            return replace(this._ordered.map(chatWindowLocalId =>
                this.env.models['discuss.chat_window'].get(chatWindowLocalId)
            ));
        }
        ...
        /**
         * @private
         * @returns {discuss.chat_window|undefined}
         */
        _computeLastVisible() {
        ...
        _computeNewMessageChatWindow() {
            const chatWindow = this.allOrdered.find(chatWindow => !chatWindow.channel);
            if (!chatWindow) {
                return unlink();
            }
        ...
        _computeUnreadHiddenConversationAmount() {
            const allHiddenWithChannel = this.allOrderedHidden.filter(
                chatWindow => chatWindow.channel
            );
            let amount = 0;
            for (const chatWindow of allHiddenWithChannel) {
                if (chatWindow.channel.localMessageUnreadCounter > 0) {
                    amount++;
                }
            }
            return amount;
    ...
    ChatWindowManager.fields = {
        ...
        // FIXME: dependent on implementation that uses arbitrary order in relations!!
        allOrdered: one2many('discuss.chat_window', {
            compute: '_computeAllOrdered',
            dependencies: [
                '_ordered',
            ],
        }),
        allOrderedChannel: one2many('discuss.channel', {
            related: 'allOrdered.channel',
        }),
        allOrderedHidden: one2many('discuss.chat_window', {
            compute: '_computeAllOrderedHidden',
            dependencies: ['visual'],
        }),
        allOrderedHiddenChannel: one2many('discuss.channel', {
            related: 'allOrderedHidden.channel',
        }),
        allOrderedHiddenChannelMessageUnreadCounter: attr({
            related: 'allOrderedHiddenChannel.localMessageUnreadCounter',
        }),
        allOrderedVisible: one2many('discuss.chat_window', {
            compute: '_computeAllOrderedVisible',
            dependencies: ['visual'],
        }),
        chatWindows: one2many('discuss.chat_window', {
            inverse: 'manager',
            isCausal: true,
        }),
        ...
        lastVisible: many2one('discuss.chat_window', {
        ...
        newMessageChatWindow: one2one('discuss.chat_window', {
        ...
        unreadHiddenConversationAmount: attr({
            ...
            dependencies: ['allOrderedHiddenChannelMessageUnreadCounter'],
        }),
        visual: attr({
    ...
    ChatWindowManager.modelName = 'discuss.chat_window_manager';

    return ChatWindowManager;
}

registerNewModel('discuss.chat_window_manager', factory);

...
function factory(dependencies) {
    ...
    class DiscussChannelCache extends dependencies['discuss.model'] {
        ...
        async loadMoreMessages() {
            ...
            if (success) {
                ...
                for (const channelView of this.channelViews) {
                    channelView.addComponentHint('more-messages-loaded', { fetchedMessages });
                }
            }
        ...
        async loadNewMessages() {
            ...
            for (const channelView of this.channelViews) {
                channelView.addComponentHint('new-messages-loaded', { fetchedMessages });
            }
        ...
        static _createRecordLocalId(data) {
            ...
            return `${this.modelName}_[${channel.localId}]_<${stringifiedDomain}>`;
        }
        ...
        _computeFetchedMessages() {
            ...
            for (const message of this.fetchedMessages) {
                if (!this.channel.messages.includes(message)) {
        ...
        _computeMessages() {
            ...
            if (!this.lastFetchedMessage) {
                newerMessages = this.channel.messages;
            } else {
                newerMessages = this.channel.messages.filter(message =>
        ...
        _computeHasToLoadMessages() {
            ...
            if (!wasCacheRefreshRequested && this.channelViews.length === 0) {
            ...
            const isMainCache = this.channel.mainCache === this;
        ...
        _extendMessageDomain(domain) {
            ...
            if (channel === this.env.messaging.inbox) {
                return domain.concat([['needaction', '=', true]]);
            } else if (channel === this.env.messaging.starred) {
                return domain.concat([
                    ['starred_partner_ids', 'in', [this.env.messaging.currentPartner.id]],
                ]);
            } else if (channel === this.env.messaging.history) {
        ...
        async _loadMessages({ extraDomain, limit = 30 } = {}) {
            ...
            this.env.messagingBus.trigger('o-channel-cache-loaded-messages', {
        ...
        /**
         * Calls "mark all as read" when this channel becomes displayed in a
        ...
        _onChangeMarkAllAsRead() {
            ...
            if (
                this.channel.isTemporary ||
                this.channel.model === 'discuss.box' ||
                this.channel.mainCache !== this ||
                this.channelViews.length === 0
            ) {
                // ignore the request
                return;
            }
        ...
        /**
         * Loads this channel cache, by fetching the most recent messages in this
         * conversation.
         *
         * @private
         */
        ...
        async _onHasToLoadMessagesChangedAsync() {
            ...
            for (const channelView of this.channelViews) {
                channelView.addComponentHint('messages-loaded', { fetchedMessages });
            }
        }

        /**
         * Handles change of messages on this channel cache. This is useful to
        ...
        _onMessagesChanged() {
            ...
            if (this.channel.mainCache !== this) {
                return;
            }
            for (const channelView of this.channel.channelViews) {
                if (channelView.channelCache) {
                    channelView.channelCache.update({ isCacheRefreshRequested: true });
                }
            }
        }
    ...
    DiscussChannelCache.fields = {
        /**
         * List of messages that have been fetched by this cache.
         *
         * This DOES NOT necessarily includes all messages linked to this
         * channel cache (@see messages field for that): it just contains list
         ...
         * fetch on this channel cache contains this message).
         *
        ...
        /**
         * Last message that has been fetched by this channel cache.
         *
         * This DOES NOT necessarily mean the last message linked to this
         ...
         *
         * This DOES NOT necessarily includes all messages linked to this
         * channel cache (@see orderedMessages field for that).
        ...
        /**
         * States the 'discuss.channel_view' that are currently displaying `this`.
         */
        channelViews: one2many('discuss.channel_view', {
            inverse: 'channelCache',
        }),
    };

    DiscussChannelCache.modelName = 'discuss.channel_cache';

    return DiscussChannelCache;
}

registerNewModel('discuss.channel_cache', factory);

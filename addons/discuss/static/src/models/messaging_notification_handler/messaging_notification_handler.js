...
import { decrement, increment, insert, link } from '@discuss/model/model_field_command';
import { htmlToTextContentInline } from '@discuss/js/utils';

const PREVIEW_MSG_MAX_SIZE = 350; // optimal for native English speakers

function factory(dependencies) {

    class MessagingNotificationHandler extends dependencies['discuss.model'] {
         ...
         *   of notification. For instance, with `discuss.channel`, this is the id
         *   of the channel.
        ...
        async _handleNotificationChannelFetched(channelId, {
            ...
            this.env.models['discuss.channel_partner_seen_info'].insert({
            ...
            this.env.models['discuss.message_seen_indicator'].recomputeFetchedValues(channel);
        }
        ...
        async _handleNotificationChannelMessage(channelId, messageData) {
            let channel = this.env.models['discuss.channel'].findFromIdentifyingData({
                id: channelId,
                model: 'discuss.channel',
            });
            ...
            if (!channel) {
                channel = (await this.async(() =>
                    this.env.models['discuss.channel'].performRpcChannelInfo({ ids: [channelId] })
                ))[0];
            }
            if (!channel.isPinned) {
                channel.pin();
            ...
            // shown on the menu, but no notification and no channel open.
            ...
            if (!isChatWithOdooBot) {
                ...
                if (channel.model === 'discuss.channel' && channel.channel_type !== 'channel') {
        ...
        async _handleNotificationChannelSeen(channelId, {
            ...
            if (shouldComputeSeenIndicators) {
                // FIXME force the computing of channel values (cf task-2261221)
                this.env.models['discuss.channel'].computeLastCurrentPartnerMessageSeenByEveryone(channel);
                // FIXME force the computing of message values (cf task-2261221)
                this.env.models['discuss.message_seen_indicator'].recomputeSeenValues(channel);
            }
        }
        ...
        _handleNotificationChannelTypingStatus(channelId, { is_typing, partner_id, partner_name }) {
            ...
            const partner = this.env.models['res.partner'].insert({
                id: partner_id,
                name: partner_name,
            });
        ...
        _handleNotificationPartnerChannel(data) {
            if (!data.members) {
                ...
                data.members = link(this.env.messaging.currentPartner);
            }
            let channel = this.env.models['discuss.channel'].findFromIdentifyingData(data);
            const wasCurrentPartnerMember = (
                channel &&
                channel.members.includes(this.env.messaging.currentPartner)
            );

            channel = this.env.models['discuss.channel'].insert(data);
            if (
        ...
        _handleNotificationPartnerMarkAsRead({ message_ids = [], needaction_inbox_counter }) {
            for (const message_id of message_ids) {
                ...
                if (message.isNeedaction) {
                    message.channel.update({ message_needaction_counter: decrement() });
                }
        ...
        _notifyNewChannelMessageWhileOutOfFocus({ channel, message }) {
            ...
            if (!author) {
                ...
                if (channel.channel_type === 'channel') {
                    ...
                    // from component
                    const channelIcon = this.env.qweb.renderToString('discuss.ThreadIcon', {
                        env: this.env,
                        channel,
                    });
        ...
        /**
         * Notifies channelViews about the given message being just received.
        ...
        _notifyThreadViewsMessageReceived(message) {
            for (const channelView of message.channel.channelViews) {
                channelView.addComponentHint('message-received', { message });
            }
        }
    ...
    MessagingNotificationHandler.modelName = 'discuss.messaging_notification_handler';

    return MessagingNotificationHandler;
}

registerNewModel('discuss.messaging_notification_handler', factory);

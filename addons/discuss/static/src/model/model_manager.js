...
import { registry } from '@discuss/model/model_core';
import { ModelField } from '@discuss/model/model_field';
import { patchClassMethods, patchInstanceMethods } from '@discuss/utils/utils';
import { unlinkAll } from '@discuss/model/model_field_command';
...
export class ModelManager {
     ...
     *
     * @param {discuss.model} Model class
     * @param {function} [filterFunc]
     * @returns {discuss.model[]} records matching criteria.
     */
     ...
     * @returns {discuss.model|discuss.model[]} newly created record(s)
     */
    create(Model, data = {}) {
     ...
     * @returns {discuss.model|undefined} the record of model matching criteria, if
     *   exists.
     */
    find(Model, findFunc) {
     ...
     * @param {discuss.model} Model class
     * @param {Object} data
     * @returns {discuss.model|undefined}
     */
     ...
     * @returns {discuss.model|undefined} record, if exists
     */
    get(Model, localId, { isCheckingInheritance = false } = {}) {
        ...
        // support for inherited models (eg. relation targeting `discuss.model`)
     ...
     * @returns {discuss.model|discuss.model[]} created or updated record(s).
     */
    insert(Model, data) {
                ...
                if (inverseField.inverse !== fieldName) {
                    throw new Error(
                        `Field name of relation "${
                            ...
                            fieldName
                        }" does not match with inverse name of relation "${
                            RelatedModel.modelName
                        }/${
                            inverseField.fieldName
                        }" ("${inverseField.inverse}" !== "${fieldName}").`
                    );
                }
     ...
     * @param {discuss.model} Model class
     * @param {Object|Object[]} [data={}]
     * @returns {discuss.model|discuss.model[]}
     */
    _create(Model, data = {}) {
    ...
    _generateModels() {
        ...
        while (toGenerateNames.length > 0) {
            ...
            if (!generatable.factory) {
                throw new Error(`Missing factory for "${generatable.modelName}".`);
            }
            ...
            for (const field of Object.values(Model.fields)) {
                for (const dependencyFieldName of field.dependencies) {
                    ...
                    while (!dependencyField) {
                        TargetModel = TargetModel.__proto__;
                        if (!TargetModel.fields) {
                            throw new Error(`Dependency "${dependencyFieldName}" of "${Model.modelName}/${field.fieldName}" does not exist.`);
                        }

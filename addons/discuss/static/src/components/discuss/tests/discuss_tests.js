...
QUnit.test('load single message from channel initially', async function (assert) {
    ...
    await this.start({
        discuss: {
            params: {
                default_active_id: 'discuss.channel_20',
            },
        },
        async mockRPC(route, args) {
            if (args.method === 'message_fetch') {
    ...
    assert.containsOnce(
        ...
        `
            .o_Discuss_thread[data-thread-local-id="${
                this.env.models['mail.thread'].findFromIdentifyingData({ id: 20, model: 'discuss.channel' }).localId
            }"]
        `,
    ...
    await this.afterEvent({
        ...
        predicate: ({ hint, threadViewer }) => {
            return (
                ...
                threadViewer.thread.model === 'discuss.channel' &&
...
QUnit.test('restore thread scroll position', async function (assert) {
    ...
    await this.start({
        discuss: {
            params: {
                default_active_id: 'discuss.channel_11',
            },
        },
        waitUntilEvent: {
            eventName: 'o-component-message-list-scrolled',
            ...
            predicate: ({ thread }) => {
                return thread && thread.model === 'discuss.channel' && thread.id === 11;
            },
        },
    });
    assert.strictEqual(
...
QUnit.test('redirect to author (open chat)', async function (assert) {
    ...
    await this.start({
        discuss: {
            params: {
                default_active_id: 'discuss.channel_1',
            },
        },
    });
    assert.ok(
...
QUnit.test('sidebar quick search', async function (assert) {
    ...
    for (let id = 1; id <= 20; id++) {
        this.data['discuss.channel'].records.push({ id, name: `channel${id}` });
    }
    await this.start();
    assert.strictEqual(
        document.querySelectorAll(`.o_DiscussSidebar_groupChannel .o_DiscussSidebar_item`).length,
...
QUnit.test('composer state: attachments save and restore', async function (assert) {
    ...
    assert.strictEqual(
        ...
        this.env.models['ir.attachment'].findFromIdentifyingData({ id: 1 }).localId,
    ...
    assert.strictEqual(
        ...
        this.env.models['ir.attachment'].findFromIdentifyingData({ id: 2 }).localId,
    ...
    assert.strictEqual(
        ...
        this.env.models['ir.attachment'].findFromIdentifyingData({ id: 3 }).localId,
    ...
    assert.strictEqual(
        ...
        this.env.models['ir.attachment'].findFromIdentifyingData({ id: 4 }).localId,
...
QUnit.test('mark channel as seen on last message visible [REQUIRE FOCUS]', async function (assert) {
    ...
    this.data['discuss.channel'].records.push({ id: 10, message_unread_counter: 1 });
    this.data['mail.message'].records.push({
        id: 12,
        body: "not empty",
        model: 'discuss.channel',
    ...
    this.data['mail.message'].records.push(
        {
            body: "not empty",
            id: 100, // random unique id, useful to link notification
            model: 'discuss.channel', // value to link message to channel
            // needaction needs to be set here for message_fetch domain, because
            // mocked models don't have computed fields
            needaction: true,
            res_id: 20, // id of related channel
...
QUnit.test('receive new chat message: out of odoo focus (notification, channel)', async function (assert) {
    ...
    // channel expected to be found in the sidebar
    // with a random unique id that will be referenced in the test
    this.data['discuss.channel'].records.push({ id: 20, channel_type: 'chat' });
    const bus = new Bus();
    ...
    await afterNextRender(() => {
        ...
        const notifications = [[['my-db', 'discuss.channel', 20], messageData]];
        this.widget.call('bus_service', 'trigger', 'notification', notifications);
    });
...
QUnit.test('receive new chat message: out of odoo focus (notification, chat)', async function (assert) {
    ...
    // and a random unique id that will be referenced in the test
    this.data['discuss.channel'].records.push({ channel_type: "chat", id: 10 });
    const bus = new Bus();
    bus.on('set_title_part', null, payload => {
        assert.step('set_title_part');
        assert.strictEqual(payload.part, '_chat');
    ...
    await afterNextRender(() => {
        ...
        const notifications = [[['my-db', 'discuss.channel', 10], messageData]];
        this.widget.call('bus_service', 'trigger', 'notification', notifications);
    });
    assert.verifySteps(['set_title_part']);
});
...
QUnit.test('receive new chat messages: out of odoo focus (tab title)', async function (assert) {
    ...
    await afterNextRender(() => {
        ...
        const notifications1 = [[['my-db', 'discuss.channel', 20], messageData1]];
    ...
    await afterNextRender(() => {
        ...
        const notifications2 = [[['my-db', 'discuss.channel', 10], messageData2]];
        this.widget.call('bus_service', 'trigger', 'notification', notifications2);
    });
    assert.verifySteps(['set_title_part']);
    ...
    await afterNextRender(() => {
        ...
        const notifications3 = [[['my-db', 'discuss.channel', 10], messageData3]];

...
class AccountJournal(models.Model):
    ...
    def _check_inbound_payment_method_line_ids(self):
        ...
        self.env['payment.provider'].flush(['code', 'state'])

        self._cr.execute('''
            SELECT provider.id
            FROM payment_provider provider
            JOIN account_payment_method apm ON apm.code = provider.code
            LEFT JOIN account_payment_method_line apml ON apm.id = apml.payment_method_id AND apml.journal_id IS NOT NULL
            WHERE provider.state IN ('enabled', 'test') AND apm.payment_type = 'inbound'
            AND apml.id IS NULL
        ...
        if ids:
            ...
            raise UserError(
                _("You can't delete a payment method that is linked to a provider in the enabled or test state.\n"
                  "Linked provider(s): %s", ', '.join(pro.display_name for pro in providers))
            )

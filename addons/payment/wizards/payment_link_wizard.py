...
class PaymentLinkWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if res_id and res_model:
            ...
            res.update(
                # TODO VFE improve method naming ?
                self.env[res_model].browse(res_id)._get_transaction_values()
            )
        return res
    ...
    partner_id = fields.Many2one('res.partner')  # why not required ?
    ...
    def _compute_link(self):
        for payment_link in self:
            ...
            for key, value in self._get_link_values().items():
                link += f'&{key}={value}'

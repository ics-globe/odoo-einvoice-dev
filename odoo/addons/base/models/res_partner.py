...
class FormatAddressMixin(models.AbstractModel):
    ...
    def _fields_view_get_address(self, node):
        ...
        if self._name == 'res.partner' and address_view_id and not self._context.get('no_address_format') and (not address_view_id.model or address_view_id.model == self._name):
            #render the partner address accordingly to address_view_id
            for address_node in node.xpath("//div[hasclass('o_address_format')]"):
                Partner = self.env['res.partner'].with_context(no_address_format=True)
                sub_view_node = Partner._fields_view_get(
                    view=address_view_id, view_type='form')
...
class Partner(models.Model):
    ...
    def _fields_view_get(self, view, view_type='form'):
        if (not view) and (view_type == 'form') and self._context.get('force_email'):
            view = self.env.ref('base.view_partner_simple_form')
        node = super(Partner, self)._fields_view_get(view=view, view_type=view_type)
        if view_type == 'form':

...
from odoo.tools import consteq, pycompat, submap
...
class IrHttp(models.AbstractModel):
    ...
    def _pre_dispatch(cls, rule, args):
        ...
        # Set a default lang
        if 'lang' not in request.env.context:
            alang = request.httprequest.accept_languages.best or 'en-US'
            try:
        ...
        try:
            if request.session.uid is not None:
                if not security.check_session(request.session, request.env):
                    raise http.SessionExpiredException("Session expired")
            getattr(cls, f'_auth_method_{endpoint.routing["auth"]}')()

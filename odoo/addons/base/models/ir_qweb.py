...
For each key-value ``t-set-key="value"`` the value will be compiled into a
python expression or into a format string expression for ``t-setf-*``.
At **rendering time** the ``values`` dictionary is updated.

(for example: ``t-set-bib="1 + 1"`` is equal to ``t-set="bib" t-value="2"``)
...
class IrQWeb(models.AbstractModel):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _render(self, template, values=None, **options):
        ...
        rendering = list(render_template(irQweb, values))
    ...
    def _eval_generate_functions(self, ref, template, code):
        try:
            ...
            return globals_dict['generate_functions']
    ...
    def _prepare_environment(self, values):
        ...
        if 'xml' in tools.config['dev_mode']:
            context['use_qweb_t_cache'] = False
        elif request and 'disable-t-cache' in request.session.debug:
            context['use_qweb_t_cache'] = False
        elif 'use_qweb_t_cache' not in self.env.context:
            context['use_qweb_t_cache'] = True
        ...
        * whose value is the content of the tag (being Markup safe).
        ...
        for key in list(el.attrib):
            if key.startswith('t-set-') or key.startswith('t-setf-'):
                ...
                varname = key.split('-', 2)
                if not VARNAME_REGEXP.match(varname[2]):
                    raise ValueError(f'The varname {varname[2]!r} can only contain alphanumeric characters and underscores.')
                if varname[1] == 'set':
                    code.append(indent_code(f"values[{varname[2]!r}] = {self._compile_expr(expr)}", level))
                else:
                    code.append(indent_code(f"values[{varname[2]!r}] = {self._compile_format(expr)}", level))
            ...
            if values.get('__qweb_in_cache'):
                ...
                for item in cache_content:
                    if is_string(item):
                        ...
                        yield ''.join(t_nocache_function(self, values))
            """, level))
    ...
    def _compile_directive_nocache(self, el, compile_context, level):
        ...
        def_code.append(indent_code("values = values['__qweb_root_values'].copy()", 1))
        def_content = self._compile_directives(el, compile_context, 1)
        if def_content and not compile_context['_text_concat']:
            self._append_text('', compile_context) # To ensure the template function is a generator and doesn't become a regular function
        def_code.extend(def_content)
        ...
        code.append(indent_code(f"""
            if values.get('__qweb_in_cache'):
                yield ({compile_context['template']!r}, {def_name!r})
            else:
    ...
    # The cache can be shared with other workers.
    @tools.conditional(
        'xml' not in tools.config['dev_mode'],
...
def render(template_name, values, load, **options):
    ...
    class MockIrQWeb(IrQWeb):
        ...
        def _prepare_environment(self, values):
            values['true'] = True
            values['false'] = False
            return self.with_context(use_qweb_t_cache=False, __qweb_loaded_values={})

    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _prepare_globals(self):
        ...
        return {
            ...
            'is_allowed_cached_value': lambda item: isinstance(item, (str, int, float, bool)),
    ...
    def _compile_directive_nocache(self, el, compile_context, level):
        ...
        if 't-nocache' not in el.attrib:
            raise SyntaxError("t-nocache-* must be on the same node of t-nocache")
        ...
        for key in list(el.attrib):
            if key.startswith('t-nocache-'):
                ...
                code_cache_values.append(indent_code(f"""
                    ...
                    if cached_value is not None and not is_allowed_cached_value(cached_value):
    ...
    def _get_cache_key(self, cache_key):
        ...
        for item in cache_key:
            try:
                ...
                keys.append(str(item.ids))
                keys.append(str(item.mapped('write_date')))
            except AttributeError:
                keys.append(str(item))
        return hashlib.sha1(';'.join(keys).encode('utf8')).hexdigest()

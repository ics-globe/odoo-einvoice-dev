...
class IrBinary(models.AbstractModel):
    ...
    def _get_stream_from(
        ...
        """
        if not record:
            raise UserError(f"Expected singleton: {record}")
        ...
        if stream.type in ('data', 'path'):
            ...
            has_extension = bool(guess_type(stream.download_name)[0])
    ...
    def _get_image_stream_from(
        ...
        This method should be preffered over :meth:`~get_stream_from`
        for images as it offers a refined caching strategy and image
        resizing capabilities.

        See also :func:`odoo.tools.image.image_process`.

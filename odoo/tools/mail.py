...
def email_normalize(text, force_single=True):
    ...
    :param boolean strict: if True, text should contain a single email. If more
      than one email is found, no normalized email is returned. If False the
      first found candidate is returned.  e.g. if email is 'tony@e.com, "Tony2"
      <tony2@e.com>' returned result is either False (force_single=True), either
      'tony@e.com' (force_single=False).
    """

...
%(fields_list)s""") % {
    ...
    def flush(self, fnames=None, records=None):
        ...
        """
        if fnames is None:
            warnings.warn("Deprecated method flush(), use env.flush_all() instead", DeprecationWarning, stacklevel=2)
            self.env.flush_all()
        elif records is None:
            warnings.warn("Deprecated method flush(), use flush_model() instead", DeprecationWarning, stacklevel=2)
            self.flush_model(fnames)
        else:
            warnings.warn("Deprecated method flush(), use flush_recordset() instead", DeprecationWarning, stacklevel=2)
            records.flush_recordset(fnames)

    def flush_model(self, fnames=None):
        """ Process the pending computations and updates on ``self``'s model.
        When the parameter is given, the method guarantees that at least the
        given fields are flushed to the database.  More fields can be flushed,
        though.
    ...
    def flush_recordset(self, fnames=None):
        """ Process the pending computations and updates on the records ``self``.
        When the parameter is given, the method guarantees that at least the
        given fields on records ``self`` are flushed to the database.  More
        fields and records can be flushed, though.
    ...
    def invalidate_cache(self, fnames=None, ids=None):
        ...
        """
        if ids is not None:
            warnings.warn("Deprecated method invalidate_cache(), use invalidate_recordset() instead", DeprecationWarning, stacklevel=2)
            self.browse(ids).invalidate_recordset(fnames)
        elif fnames is not None:
            warnings.warn("Deprecated method invalidate_cache(), use invalidate_model() instead", DeprecationWarning, stacklevel=2)
            self.invalidate_model(fnames)
        else:
            warnings.warn("Deprecated method invalidate_cache(), use env.invalidate_all() instead", DeprecationWarning, stacklevel=2)
            self.env.invalidate_all()
    ...
    def invalidate_recordset(self, fnames=None):
        """ Invalidate the cache of the records in ``self``.  When the parameter
    ...
    def recompute(self, fnames=None, records=None):
        ...
        """
        if fnames is None:
            warnings.warn("Deprecated method recompute(), use env.flush_all() instead", DeprecationWarning, stacklevel=2)
            self.env._recompute()
        elif records is None:
            warnings.warn("Deprecated method recompute(), use flush_model() instead", DeprecationWarning, stacklevel=2)
            self._recompute_model(fnames)
        else:
            warnings.warn("Deprecated method recompute(), use flush_recordset() instead", DeprecationWarning, stacklevel=2)

...
class Stream:
    ...
    def from_path(cls, path):
        ...
        return cls(
            type='path',
            path=path,
            etag=f'{int(stat.st_mtime)}-{stat.st_size}-{check}',  # that's what send_file does
            size=stat.st_size,
    ...
    def get_response(self, as_attachment=None):
        ...
        send_file_kwargs = {
            ...
            'as_attachment': self.as_attachment if as_attachment is None else as_attachment,
            ...
            'use_x_sendfile': config['x_sendfile']
        }
        ...
        if self.type == 'path':
            ...
            if 'x-sendfile' in res.headers:
                res.headers['x-accel-redirect'] = f'/web/filesystem{self.path}'
            return res

...
Request._serve_ir_http
  ...
  can be extended in modules. This model is responsive of:
...
Request.json_dispatch
  ...
  implementation of jsonrpc2, it only  the ``params``
...
class Request:
    ...
    def csrf_token(self, time_limit=DEFAULT_CSRF_TOKEN_LIFETIME):
        ...
        :returns str: ASCII token string
        """
    ...
    def validate_csrf(self, csrf):
        ...
        :returns bool: ``True`` when valid, ``False`` when not.
        """
        if not csrf:
            return False
    ...
    #------------------------------------------------------
    def _json_dispatch(self, endpoint):
        """
    ...
    def _serve_nodb(self):
        ...
        """
        router = self.app.nodb_routing_map.bind_to_environ(self.httprequest.environ)
    ...
    def _serve_db(self, dbname, session_id):
        ...
        self.session_id = session_id or secrets.token_urlsafe()
    ...
    def __call__(self, environ, start_response):
        ...
        if len(segments) >= 4 and segments[2] == 'static':
            ...
            if dbname is None:
                ...
                response = request._dispatch_db(dbname, sid)
